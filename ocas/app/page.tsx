...
export default function Home() {
  ...
  const { isSignedIn, isLoaded, user } = useUser(); // Use `useUser` hook for user data
  const [userRole, setUserRole] = useState<string | null>(null);

  useEffect(() => {
    if (isLoaded && user) {
      const role = user.publicMetadata?.role as string | undefined; // Retrieve role from public metadata
      setUserRole(role ?? "user"); // Default to "user" if no role is found
    }
  }, [isLoaded, user]);
        ...
        <section className="relative h-screen flex items-center justify-center">
          ...
          <div className="relative z-10 text-center px-6">
            ...
            >
              ...
              >
                {!isSignedIn ? "Get Started" : userRole === "admin" ? "Admin Panel" : "Access Dashboard"}
              </Button>
            </motion.div>
          </div>
        </section>
...
//           onClick={() => router.push("/login")}
//           size="lg"
//           className="bg-blue-600 hover:bg-blue-700 text-white rounded-full shadow-lg hover:shadow-xl transition duration-300"
//         >
//           Login Now
//         </Button>
//       </motion.div>
